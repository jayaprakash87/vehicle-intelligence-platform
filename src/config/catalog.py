...
# ---------------------------------------------------------------------------
# Sources: Infineon PROFET+ datasheets, NXP MC33xxx series, typical OEM specs.
# r_ds_on: on-resistance falls with higher current ratings (bigger die)
# r_thermal_kw: thermal resistance in °C/W (junction-to-ambient, with PCB)
...
EFUSE_CATALOG: dict[EFuseFamily, EFuseProfile] = {
    EFuseFamily.HS_2A: EFuseProfile(
        ...
        cooldown_s=0.5, max_retries=5, adc_bits=12, load_type="resistive",
    ),
    EFuseFamily.HS_5A: EFuseProfile(
        ...
        cooldown_s=0.5, max_retries=4, adc_bits=12, load_type="resistive",
    ),
    EFuseFamily.HS_10A: EFuseProfile(
        ...
        cooldown_s=1.0, max_retries=3, adc_bits=12, load_type="resistive",
    ),
    ...
    EFuseFamily.HS_20A: EFuseProfile(
        ...
        cooldown_s=1.5, max_retries=3, adc_bits=12, load_type="resistive",
    ),
    EFuseFamily.HS_30A: EFuseProfile(
        ...
        cooldown_s=2.0, max_retries=2, adc_bits=12, load_type="resistive",
    ),
    EFuseFamily.HS_50A: EFuseProfile(
        ...
        cooldown_s=3.0, max_retries=2, adc_bits=10, load_type="resistive",
    ),
...
def sedan_topology() -> tuple[list[ZoneController], list[dict]]:
    ...
    zones = [
        ...
        ZoneController(zone_id="zone_underhood", name="Underhood Zone Controller", location="underhood"),
    ]
